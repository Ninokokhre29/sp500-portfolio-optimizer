...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Overview", "Market Analysis", "Portfolio Strategy 1","Portfolio Strategy 2", "Performance"])
    ...
    with tab4:
        ...
        with col2:
            ...
            for _, row in regular_month.iterrows():
                stock_amt = amount * row["weight"]
                st.markdown(f"- {row['index']}: ${stock_amt:,.2f} ({row['weight']:.2%})")
            gain_hist = amount * (hist_return / 100)     
    ...
    with tab5:
        ...
        if 'methodology' in annual_df.columns and 'portfolio return' in annual_df.columns:
            ...
            data = { "methodology": ["MV", "MV + ARIMA"],  "portfolio return": [8.91, 10.16], "Sharpe Ratio": [0.16, 0.39]}
